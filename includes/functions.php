...
if (!function_exists('wpguppy_OnlineInit')) {
	...
	function wpguppy_OnlineInit(){
		...
		$logged_in_users = is_array($logged_in_users) ? $logged_in_users : array(); 
		...
		switch ($column_name) {
			case 'guppy_action' :
				...
				<div class="db-guppy-whatsappcheck">
					...
					<a href="javascript:void(0);" class="guppy_whatsapp_user_edit"  data-id="<?php echo esc_attr($user_id);?>"><?php echo esc_html('Edit', 'wpguppy-lite'); ?></a>
				</div>
...
if (!function_exists('guppywhatsappPopup')) {
	...
	if(is_plugin_active('wpguppy-lite/wpguppy-lite.php')){
		add_action('admin_footer-users.php', 'guppywhatsappPopup');
			...
			<div class="db-guppy-cus-modal">
				<div class="db-guppy-cus-modal-dialog">
					...
					<div class="db-guppy-cus-modal-header">
						<a href="javascript:void(0);"  class="db-guppy-cus-close-modal">×</a>
