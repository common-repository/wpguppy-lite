...
<table class="form-table"  role="chat">
    <tbody>
        <tr>
            <th scope="row"><label for="group"><?php esc_html_e('Chat options','wpguppy-lite');?></label></th>
            <td>
                <fieldset>
                    ...
                    <code><?php esc_html_e('Pusher channel','wpguppy-lite');?></code></label>
                    ...
                    <code><?php esc_html_e('Socket-io','wpguppy-lite');?></code></label>
                </fieldset>
            </td>
        </tr>
        <tr class="rt-pusher <?php if( empty($rt_chat_settings) || $rt_chat_settings != 'pusher') { echo esc_attr($pusher_class); } ?>">
        ...
        <tr class="rt-socket <?php if( empty($rt_chat_settings) || $rt_chat_settings != 'socket') { echo esc_attr($socket_class); } ?>">
            <th scope="row"><label for="group"><?php esc_html_e('Socket io settings','wpguppy-lite');?></label></th>
            <td>
                <fieldset>
        ...
        <tr class="gp-pusher-options <?php echo esc_attr($pusher_class);?>">
            <td colspan="2"><p><?php echo do_shortcode('We have used <a href="https://pusher.com/channels">Pusher Channel API</a> for the real-time chat experience, you must create channel API keys and add into the below settings and need to enable client events in the pusher app settings.  You may <a href="https://www.youtube.com/watch?v=70JOw13QKk8&list=PLuU227tzkYJEix0c4hOmUuo0wYf_4LWc2">check this guide</a>','wpguppy-lite');?></p></td>
        </tr>
        <tr class="gp-pusher-options <?php echo esc_attr($pusher_class);?>">
        ...
        <tr class="gp-socket-options <?php echo esc_attr($socket_class);?>">
            <th scope="row"><label for="gp-host-url"><span><?php esc_html_e('Host url','wpguppy-lite');?></span></label></th>
            <td>
                ...
                <p><?php esc_html_e('Please add the host url, it could be https://yourdomain.com','wpguppy-lite');?></p>
            </td>
        </tr>
        <tr class="gp-socket-options <?php echo esc_attr($socket_class);?>">
            <th scope="row"><label for="gp-port-id"><span><?php esc_html_e('Port ID','wpguppy-lite');?></span></label></th>
            <td>
            ...
            <p><?php esc_html_e('Please add the available port for node server, default would be 81.','wpguppy-lite');?></p>
            <p><?php esc_html_e('1) Some server uses 80, 81, 8080 or 3000.','wpguppy-lite');?></p>
            <p><?php esc_html_e('2) Please consult with your hosting provider.','wpguppy-lite');?></p>
            <p><?php esc_html_e('3) You need to install pm2 globally on your hosting server through this command npm install pm2 -g for this, you can contact your hosting service provider or your server manager.','wpguppy-lite');?></p>
            <p><?php esc_html_e('4) You have to add your port id in .env file located in plugins > wp-guppy > node-server > .env at line PORT_ID.','wpguppy-lite');?></p>
            <p><?php esc_html_e('5) You have to add your host url as CORS origin in .env file located in plugins > wp-guppy > node-server > .env at line DOMIAN.','wpguppy-lite');?></p>
            <p><?php esc_html_e('6) No need to change the port if your server is using port 81. (if you will change this port then you have to change it in the .env file located in plugins > wp-guppy > node-server > .env at line PORT_ID).','wpguppy-lite');?></p>
            <p><?php esc_html_e('7) You need to run the node server file (located in plugins > wp-guppy > node-server) on the server through server access like (SSH access), for this, you can contact your hosting service provider or your server manager. if you have an HTTP server then you need to just run the server.js file through the command ( pm2 start server.js ), but if you have an HTTPS server then you have to download an SSL certificate (SSL cert, SSL private key ) from your server then replace these files located in plugins > wp-guppy > node-server > sslcert with the same name given in that folder, after that you need to run command (pm2 start server-ssl.js) at (plugins > wp-guppy > node-server) path.','wpguppy-lite');?></p>
            <p><?php esc_html_e('8) If the plugin update is available for node server packages than you need to run command(npm update) at (plugins > wp-guppy > node-server) path.','wpguppy-lite');?></p>
            <p><?php esc_html_e('9) If the plugin update is available for node server then you need to run command(pm2 restart server.js) for HTTP servers and command(pm2 restart server-ssl.js) for HTTPS servers at (plugins > wp-guppy > node-server) path.','wpguppy-lite');?></p>
            </td>
        </tr>
    </tbody>
</table>